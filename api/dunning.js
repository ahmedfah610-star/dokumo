...
// Scalony z funkcji compliance-radar.js w jedną funkcję serverless (limit
// 12 funkcji na planie Hobby) — routing po action==='compliance-check'.
...
const RADAR_DEBUG = process.env.COMPLIANCE_RADAR_DEBUG_MODE === 'true';
...
async function classifyRadarAct(act, typeIds) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      console.error('classifyRadarAct AI error:', e.message);
    }
  }
...
async function sendRadarEmail({ to, subject, body }) {
  if (RADAR_DEBUG) return 'debug-mode';
  const key = process.env.RESEND_API_KEY;
  if (!key) throw new Error('Brak RESEND_API_KEY');
  const r = await fetch('https://api.resend.com/emails', {
    method: 'POST',
...
async function detectNewAlerts() {
  ...
  for (const act of acts) {
    ...
    const cls = await classifyRadarAct(act, typeIds);
    const record = {
      eli: act.ELI || null,
      title: act.title,
      type: act.type,
...
Zespół Dokumo`;
    try {
      await sendRadarEmail({ to: ownerEmail, subject, body });
    } catch (e) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'compliance-check' && cronAuthed) {
      ...
      return res.status(200).json({ ok: true, debug: RADAR_DEBUG, newAlerts: alerts.length, usersChecked, docsFlagged });
    }
